...
const ROLE_COLORS  = { admin: 'text-red-400 bg-red-900/20 border-red-700/40', cashier: 'text-blue-400 bg-blue-900/20 border-blue-700/40', waiter: 'text-green-400 bg-green-900/20 border-green-700/40' };

function StaffTab() {
    const [staffList, setStaffList] = useState([]);
    const [loading, setLoading]     = useState(false);
    const [form, setForm]           = useState(null); // null | { id?, name, role, pin, email, phone }
    ...
    const openAdd = () => setForm({ name: '', role: 'cashier', pin: '', email: '', phone: '' });
    const openEdit = (s) => setForm({ id: s.id, name: s.name, role: s.role, pin: s.pin, email: s.email || '', phone: s.phone || '' });
                        ...
                        <div key={s.id} className="bg-gray-800 border border-gray-700/50 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
                            <div className="min-w-0">
                                ...
                                <p className="text-gray-500 text-xs mt-0.5">
                                    PIN: {'•'.repeat(s.pin?.length || 4)}
                                    {s.email ? ` · ${s.email}` : ''}
                                    {s.last_login ? ` · Last login: ${new Date(s.last_login + 'Z').toLocaleDateString('en-LK')}` : ''}
                                </p>
                            </div>
                    ...
                    <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-sm shadow-2xl p-6 space-y-4">
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <input type="text" value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} maxLength={50}
                            ...
                            <div>
                                <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1 block">PIN * (4-6 digits)</label>
                                <input type="password" inputMode="numeric" value={form.pin} onChange={e => { if (/^\d{0,6}$/.test(e.target.value)) setForm(f => ({...f, pin: e.target.value})); }} maxLength={6}
