...
const MiniCalendar = ({ value, rangeFrom, rangeTo, onSelect, selecting }) => {
    ...
    return (
        <div className="border border-white/[0.08] rounded-2xl p-4 w-64 shadow-2xl" style={{ background: '#141922' }}>
            {/* Nav */}
            <div className="flex items-center justify-between mb-3">
                <button onClick={prevMonth} className="w-7 h-7 rounded-lg hover:bg-white/10 flex items-center justify-center text-gray-400 hover:text-gray-100 transition-colors">
                ...
                <span className="text-gray-100 font-bold text-sm">{MONTHS[viewMonth]} {viewYear}</span>
                <button onClick={nextMonth} className="w-7 h-7 rounded-lg hover:bg-white/10 flex items-center justify-center text-gray-400 hover:text-gray-100 transition-colors">
                        ...
                        <button
                            ...
                            className={`
                                ...
                                ${isFrom || isTo || isSelected
                                    ? 'bg-blue-600 text-gray-100 shadow-lg shadow-blue-900/40'
                                    : inRange
                                        ? 'bg-blue-500/15 text-blue-300'
                                        : isFuture ? '' : 'text-gray-400 hover:bg-white/10 hover:text-gray-100'}
                            `}
                        >
                            {d}
    ...
    return (
        <div ref={ref} className="relative">
            <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 border border-gray-700/50 hover:border-blue-500/40 rounded-xl text-sm text-gray-100 font-medium transition-all min-w-[220px]"
            ...
            {open && (
                <div className="absolute top-full mt-2 right-0 z-50 flex gap-4 border border-white/[0.08] rounded-2xl p-4 shadow-2xl" style={{ background: '#141922' }}>
                    {/* From calendar */}
                    <div>
...
const StatCard = ({ label, value, sub, accent, icon }) => (
    <div className="border border-gray-700/30 rounded-2xl p-5 flex items-start gap-4" style={{ background: '#141922' }}>
        <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${accent}`}>
            {icon}
        </div>
        <div className="min-w-0">
            <p className="text-gray-500 text-xs font-medium uppercase tracking-wider">{label}</p>
            <p className="text-gray-300 font-bold text-lg mt-1 truncate tabular-nums">{value}</p>
            {sub && <p className="text-gray-500 text-xs mt-0.5">{sub}</p>}
        </div>
    </div>
);

const HBarChart = ({ data, valueKey, labelKey, colorClass = 'bg-blue-500/70', formatValue }) => {
            ...
            {data.map((row, i) => {
                ...
                return (
                    <div key={i} className="flex items-center gap-3">
                        ...
                        <span className="text-gray-400 text-xs w-28 shrink-0 text-right tabular-nums">
...
const ReportsView = () => {
    ...
    return (
        <div className="h-full flex flex-col" style={{ background: '#0f1319' }}>
            {/* ── Header ── */}
            <div className="border-b border-white/[0.05] px-6 py-4 shrink-0 flex items-center justify-between" style={{ background: '#0f1319' }}>
                <div>
                    <h2 className="text-gray-100 font-bold text-lg">Reports</h2>
                ...
                <button
                    ...
                    disabled={loading}
                    className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-gray-100 rounded-xl text-sm font-semibold transition-colors"
                >
                        ...
                        {PRESETS.map((p, i) => (
                            <button
                                ...
                                className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-all
                                    ${preset === i
                                        ? 'bg-blue-600/80 text-gray-100 shadow shadow-blue-900/40'
                                        : 'bg-gray-800/60 border border-gray-700/40 text-gray-400 hover:text-gray-200 hover:bg-gray-800/80'}`}
                            >
                                {p.label}
                            </button>
                        ))}
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            <StatCard
                                ...
                                accent="bg-emerald-500/8 border border-emerald-500/15"
                            ...
                            <StatCard
                                ...
                                accent="bg-blue-500/8 border border-blue-500/15"
                            ...
                            <StatCard
                                ...
                                accent="bg-purple-500/8 border border-purple-500/15"
                            ...
                            <StatCard
                                ...
                                accent="bg-amber-500/8 border border-amber-500/15"
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            {/* ── Top Items ── */}
                            <div className="border border-gray-700/30 rounded-2xl p-5" style={{ background: '#141922' }}>
                                <h3 className="text-gray-200 font-semibold text-sm flex items-center gap-2 mb-5">
                                            ...
                                            <div key={i} className="flex items-center gap-3">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        ...
                                                        <span className="text-gray-500 text-xs w-24 text-right shrink-0 tabular-nums">{lkr(item.revenue)}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                        ...
                                        <div className="grid grid-cols-3 gap-2 pt-3 border-t border-white/[0.05]">
                                            {byMethod.map((m, i) => (
                                                <div key={i} className="text-center bg-white/[0.02] rounded-xl py-2.5">
                                                    <p className="text-gray-100 font-black text-lg tabular-nums">{m.count}</p>
                                                    <p className="text-gray-500 text-xs capitalize mt-0.5">{m.payment_method || 'Unknown'}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    {Array.from({ length: 24 }, (_, h) => {
                                        ...
                                        return (
                                            <div key={h} className="flex-1 flex flex-col items-center gap-1" title={`${fmtHour(h)}: ${lkr(rev)}`}>
                                                <div className="w-full rounded-t-md bg-gray-700/30 relative overflow-hidden" style={{ height: 96 }}>
                                                    <div
                                                        className="absolute bottom-0 w-full bg-indigo-400/80 rounded-t-md transition-all duration-700"
                            ...
                            <div className="border border-gray-700/30 rounded-2xl p-5" style={{ background: '#141922' }}>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {byType.map((t, i) => {
                                        ...
                                        const cols = [
                                            { bar: 'bg-blue-500/70', ring: 'border-blue-500/20', bg: 'bg-blue-500/4' },
                                            { bar: 'bg-amber-500/70', ring: 'border-amber-500/20', bg: 'bg-amber-500/4' },
                                        ];
                                        const c = cols[i % cols.length];
                                        return (
                                            <div key={i} className={`${c.bg} border ${c.ring} rounded-2xl p-5 text-center`}>
                                                <p className="text-gray-100 font-black text-3xl tabular-nums">{t.count}</p>
                                                <p className="text-gray-400 text-sm capitalize font-medium mt-1">{t.order_type}</p>
                                                <p className="text-gray-500 text-xs mt-0.5">{pct}% of orders</p>
                                                <div className="mt-3 bg-gray-700/40 rounded-full h-1.5 overflow-hidden">
                                                    <div className={`h-full ${c.bar} rounded-full transition-all duration-700`} style={{ width: `${pct}%` }} />
                                                </div>
                                                <p className="text-gray-200 font-bold text-sm mt-2 tabular-nums">{lkr(t.revenue)}</p>
                                            </div>
                                        );
                                    })}
                                </div>
